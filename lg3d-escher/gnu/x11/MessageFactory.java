...
public class MessageFactory {
  public static Error build_connection_error (Display display) 
    ...
    EscherDataInputStream din = display.connection.din;
  ...
  public static Error build_extension_error (Display display, Data data, 
    ...
    if (factory == null) {
      // TODO: temporary work around for bug 157. Print a warning for 
      // this condition, but don't make it fatal
      //throw new java.lang.Error ("Unsupported extension error: " + code);
      System.err.println("WARNING: MessageFactory error in thread " + Thread.currentThread().getName());
      System.err.println("WARNING: MessageFactory.build_extension_error: Unsupported extension event: " + code);
      return null;
    }
  ...
  public static Event build_extension_event (Display display, 
    ...
    if (factory == null) {
      ...
      //throw new java.lang.Error ("Unsupported extension error: " + code);
      System.err.println("WARNING: MessageFactory error in thread " + Thread.currentThread().getName());
      System.err.println("WARNING: MessageFactory.build_extension_event: Unsupported extension event: " + code);
      return null;
    }
  ...
  public static Data build (Display display) throws IOException {
    ...
    // Note: setSoTimeout is a no-op on Solaris/Linux.
    //display.connection.setSoTimeout (0);
     ...
     * not be timed out.
     *
     display.connection.setSoTimeout (10000);
     */


    if (code == 0) { 		// X Error
      Error error = build_error (display);
      if (error == null) {
        return null;
      } else {
        throw error;
  ...
  public static Data build_reply (Display display) throws IOException {
    ...
    display.connection.din.readFully (
