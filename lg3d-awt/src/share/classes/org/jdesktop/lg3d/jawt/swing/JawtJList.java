 ...
 */
public class JawtJList extends JList implements JawtDelegateInterface {

    List target;
    DefaultListModel listModel;

    public JawtJList(List target) {
	super();
	this.target = target;

	setModel(listModel = new DefaultListModel());
	for (int i = 0; i < target.getItemCount(); i++)
	    listModel.addElement(target.getItem(i));
    }
