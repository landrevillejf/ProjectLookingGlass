...
| Start-menu name / group | Natural Language Control / **Utilities** — descriptor **`lg3d-apps/src/config/nlc.lgcfg`** → `config/demo` (**discovered**) |
...
- **Architect** — An event-based voice/natural-language controller: it parses speech and
  ...
  own. It is **registered** (descriptor in `lg3d-apps`) but cannot fully run
...
- **Project Manager** — Commit scope `lg3d-incubator`; descriptor lives in
  `lg3d-apps` (PR may span two modules). Track the `ext/javanlp` run-classpath
