/*
 * $Header: /zpool01/javanet/scm/svn/tmp/cvs2svn/lg3d-incubator/src/classes/org/jdesktop/lg3d/apps/kwebdemo1/components/AgesInHistoryC3D.java,v 1.4 2006-12-13 22:01:36 paulby Exp $
...
/**
 * The component that handles the "Ages in History" flying title animation
 * during the startup sequence.
 * 
 * This class brings the title into and out of view and positions it in the
 * right place in the scene.  It also setups up event listeners, actions that
 ...
 */
public class AgesInHistoryC3D extends Component3D
{
  private Application app = null;
  private BranchGroup agesInHistoryBG = null;
  private AnimationGroup agesInHistoryAG = null;
  
  private TranslationAnimationBoolean agesInHistoryTA;
   ...
   */
  public AgesInHistoryC3D(Application app)
  {
    super();
    this.app = app;
    agesInHistoryBG = new BranchGroup();
    agesInHistoryAG = createLabel();
    agesInHistoryBG.addChild(agesInHistoryAG);
    this.addChild(agesInHistoryBG);
  }

  /**
   * Runs an animation that flys the title into view. And, it sets up an
  ...
  public void zoomIntoView(int millisecs, int nextStep)
  {
    agesInHistoryTA =
      new TranslationAnimationBoolean(new Vector3f(0f, 0f, 0.8f),
                                      new Vector3f(-0.012f, 0.04f, 0.2f),
                                      millisecs);
    agesInHistoryAG.setAnimation(agesInHistoryTA);
    LgEventConnector.getLgEventConnector().addListener(agesInHistoryTA,
            new GenericEventAdapter(LgEvent.class,
                                    new StartupSequenceAction(app, nextStep))
            );
    agesInHistoryTA.setAnimationFinishedEvent(LgEvent.class);
    agesInHistoryTA.performAction(null, true);
  }
  
  /**
   * Runs an animation that flys the title out of view and makes it invisible.
  ...
  {
    zoomOutOfViewTA =
      new TranslationAnimationBoolean(new Vector3f(-0.012f, 0.04f, 0.2f),
                                      new Vector3f(0f, 0f, 0.8f),
                                      millisecs,
                                      new AcceleratingVector3fSmoother());
    agesInHistoryAG.setAnimation(zoomOutOfViewTA);
   ...
   * <code>ImagePanel</code> that has the "Ages in History" title textured
   * onto it.
   * 
  ...
  private AnimationGroup createLabel()
  {
    ...
    try
      {
        URL imageFilename 
          = getClass().getResource("/org/jdesktop/lg3d/apps/kwebdemo1/resources/images/AgesInHistory.png");
        ImagePanel splash 
          = new ImagePanel(imageFilename, 0.1f, 0.0125f);
        ag.addChild(splash);
      }
    catch (FileNotFoundException e)
      {
        System.err.println("Kwebdemo1 AgesInHistoryC3D createLabel(): " +
                           e.toString());
      }
    catch (IOException e)
      {
