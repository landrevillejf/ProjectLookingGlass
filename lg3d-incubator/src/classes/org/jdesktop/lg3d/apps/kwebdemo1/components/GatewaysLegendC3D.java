/*
 * $Header: /zpool01/javanet/scm/svn/tmp/cvs2svn/lg3d-incubator/src/classes/org/jdesktop/lg3d/apps/kwebdemo1/components/GatewaysLegendC3D.java,v 1.4 2006-12-13 22:01:39 paulby Exp $
...
/**
 * The component that handles the "Gateways into History" flying title animation
 ...
 */
public class GatewaysLegendC3D extends Component3D
{
  private Application app = null;
  private BranchGroup gatewaysBG = null;
  private AnimationGroup gatewaysAG = null;
   ...
   */
  public GatewaysLegendC3D(Application app)
  {
    super();
    this.app = app;
    gatewaysBG = new BranchGroup();
    gatewaysAG = createLabel();
    gatewaysBG.addChild(gatewaysAG);
    this.addChild(gatewaysBG);
  }
  ...
  public void zoomIntoView(int millisecs, int nextStep)
  {
    zoomIntoViewTA =
      new TranslationAnimationBoolean(new Vector3f(0f, 0f, 0.8f),
                                      new Vector3f(0.023f, 0.03f, 0.2f),
                                      millisecs);
    gatewaysAG.setAnimation(zoomIntoViewTA);
  ...
  {
    zoomOutOfViewTA =
      new TranslationAnimationBoolean(new Vector3f(0.023f, 0.03f, 0.2f),
                                      new Vector3f(0f, 0f, 0.8f),
                                      millisecs,
                                      new AcceleratingVector3fSmoother());
    gatewaysAG.setAnimation(zoomOutOfViewTA);
   ...
   * <code>ImagePanel</code> that has the "Gateways into History" title textured
   * onto it.
   * 
  ...
  private AnimationGroup createLabel()
  {
    ...
    try
      {
        URL imageFilename 
          = getClass().getResource("/org/jdesktop/lg3d/apps/kwebdemo1/resources/images/GatewaysIntoHistory.png");
        ImagePanel splash 
      ...
      {
        System.err.println("Kwebdemo1 GatewaysLegendC3D createLabel(): " +
                           e.toString());
      }
    catch (IOException e)
      {
