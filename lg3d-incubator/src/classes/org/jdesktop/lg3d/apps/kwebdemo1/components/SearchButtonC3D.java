/*
 * $Header: /zpool01/javanet/scm/svn/tmp/cvs2svn/lg3d-incubator/src/classes/org/jdesktop/lg3d/apps/kwebdemo1/components/SearchButtonC3D.java,v 1.4 2006-12-13 22:01:40 paulby Exp $
...
/**
 * The component that handles the Search button animation at the begining of
 * the startup sequence and all the UI actions with the button.
 * 
 * <p>The purpose of the Search button is to show a popup dialog that allows
 * the user to either enter part of the name of the person, thing or event
 * in history that he/she wants to start a journey; or, find the person, thing
 * or event by a nested category search.</p>
 * 
 ...
 */
public class SearchButtonC3D extends Component3D
{
   ...
   */
  public SearchButtonC3D(Application app)
  {
    super();
    this.app = app;
    ...
    addMouseReleasedListener(app, "Search");
  ...
  public void zoomIntoView(int millisecs, int nextStep)
  {
    buttonTA =
      new TranslationAnimationBoolean(new Vector3f(0f, 0f, 0.8f),
                                      new Vector3f(-0.125f, 0.1f, -0.15f),
                                      millisecs);
  ...
  private AnimationGroup createIcon()
  {
    ...
    try
      {
        URL iconNFilename 
          = getClass().getResource("/org/jdesktop/lg3d/apps/kwebdemo1/resources/images/icon/SearchN_64x64.png");
        buttonIP 
  ...
  private void addMouseRolloverListener()
  {
    ...
    try
      {
        URL iconRFilename 
          = getClass().getResource("/org/jdesktop/lg3d/apps/kwebdemo1/resources/images/icon/SearchR_64x64.png");
        app.setTexture(iconRFilename);
      ...
      {
        System.err.println(
                "Kwebdemo1 SearchButtonC3D addMouseRolloverListener(): " +
                e.toString());
      }
    catch (IOException e)
      {
  ...
  try
    {
        URL iconSFilename 
          = getClass().getResource("/org/jdesktop/lg3d/apps/kwebdemo1/resources/images/icon/SearchS_64x64.png");
      app.setTexture(iconSFilename);
    ...
    {
      System.err.println(
              "Kwebdemo1 SearchButtonC3D addMousePressedListener(): " +
              e.toString());
    }
  catch (IOException e)
    {
