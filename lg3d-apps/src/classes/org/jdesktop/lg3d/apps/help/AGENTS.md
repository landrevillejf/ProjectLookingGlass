...
- **Full-text search** - the Search navigator needs a generated `JavaSearch`
  database. The `:lg3d-apps:generateHelpSearchIndex` task runs JavaHelp's
...
- **Dependencies** - JavaHelp is declared in `gradle/libs.versions.toml`, added to
  `lg3d-apps` as `implementation`, and resolved onto the hand-assembled
...
Launch standalone:
```bash
./gradlew :lg3d-apps:run -Papp=help
```
