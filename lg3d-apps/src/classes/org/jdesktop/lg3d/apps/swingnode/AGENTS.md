...
- **Functional Analyst** — Spec as a demonstration (render a Swing panel onto
  ...
  `docs/swingnode.md`.
- **Project Manager** — Commit scope `lg3d-apps`. Because it documents a core
...
the same PR. Commit scope `lg3d-apps`; add a `CHANGELOG.md` bullet under
...
Launch standalone:
```bash
./gradlew :lg3d-apps:run -Papp=swingnode
```
