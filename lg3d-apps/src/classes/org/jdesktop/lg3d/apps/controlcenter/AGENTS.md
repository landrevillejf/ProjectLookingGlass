...
- **Architect** — Production settings hub hosted through `TitledSwingWindow`/
  `SwingNode` from `lg3d-core`; the `ControlPanel` interface + `ControlPanelRegistry`
...
- **Engineer / Developer** — Follow the core UI/UX rulebook: offscreen SwingNode
  ...
  `installHostedLookAndFeel`. Add a panel by implementing the `ControlPanel`
  interface and registering it in `ControlPanelRegistry` - no host or tab edits.
...
- **UI/UX (3D & 2D)** — 3D: glassy `TitledSwingWindow` frame (title strip,
  decoration buttons, transparency ordering). 2D: the nav-list + `CardLayout` Swing
  panel, identical in the 2D desktop. Verify overlay ordering over a maximized window.
...
- Showcase a navigation-list + card layout in the 3D environment

## Key Components

- **ControlCenter** - Main entry point, creates Frame3D and SwingNode
- **ControlCenterPanel** - Swing JPanel: a `JList` navigation list + `CardLayout`
...
1. Implement the `ControlPanel` interface
2. Register it in `ControlPanelRegistry.panels()` via `addDefault(...)`

No host or tab edits are needed - `ControlCenterPanel` builds its navigation list
and cards from the registry.

```java
public class MyPanel implements ControlPanel {
    public String displayName() { return "My Settings"; }
    public javax.swing.Icon icon() { return null; }
    public javax.swing.JComponent component() { /* build once, reuse */ }
    // onShow()/onHide() have empty defaults; override to refresh live state
}
```
...
- Java Swing: JPanel, JList, CardLayout, standard Swing components
...
- **Panel Layout**: Modify `ControlCenterPanel` for a different nav/card arrangement
- **Settings Storage**: `DesktopConfig` persists the desktop-side settings; the
  system panels persist through their own OS tools (timedatectl, localectl, xset,
  bluetoothctl)

## Known Limitations

- Panels are limited to `JList` / `JCheckBox` / `JButton` / `JTextField` - never a
